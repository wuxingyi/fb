...
struct lease_grant_response_t {
  ...
  bool is_ok() const { return statusCode == status_code_e::OK; }
};
...
struct kv_range_response_t {
  ...
  explicit kv_range_response_t(status_code_e code) : statusCode(code) {}
...
struct kv_list_response_t {
  ...
  explicit kv_list_response_t(status_code_e code) : statusCode(code) {}
